...
sudo tee /etc/default/fluent-bit > /dev/null << EOT
OBSERVE_HOST=${OBSERVE_ENDPOINT}
OBSERVE_TOKEN=${OBSERVE_TOKEN}
EOT

curl https://raw.githubusercontent.com/yasar-observe/host-configuration-scripts/yasar/init/fluent-bit/linux/observe_fluent_install.sh | bash -s -- --observe_host "${OBSERVE_ENDPOINT}" --observe_token "${OBSERVE_TOKEN}"