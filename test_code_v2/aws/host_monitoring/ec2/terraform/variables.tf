...
variable "windows_machines" {
  ...
  default = {
    WINDOWS_SERVER_2022_BASE_OTEL_AGENT_INSTALLED = {
      ...
      host_mon_user_data_path = "user_data/windows.ps"
      otel_user_data_path     = "user_data/windows_otel.ps"
      no_agent_user_data_path = "user_data/windows_noagentinstall.ps"
    }
...
locals {
  ...
  SCRIPTS = {
    ...
    windows = <<EOT
    "${local.PRE}/windows-host-configuration-scripts/${local.BRANCH["host_mon"]}/agents.ps1" -outfile .\agents.ps1; .\agents.ps1  -ingest_token ${local.TOKENS["host_mon"]}  -customer_id ${var.OBSERVE_CUSTOMER}
    EOT
  }
